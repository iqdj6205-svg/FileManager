...
class WearBridgeListenerService : WearableListenerService() {
    override fun onMessageReceived(messageEvent: MessageEvent) {
        ...
        Wearable.getMessageClient(this).sendMessage(
            ...
            WearBridgePaths.MESSAGE_COMMAND_RESULT,
            WearBridgeCodec.encode(result),
        )
    }
    ...
    private fun handle(command: WearBridgeCommand): WearBridgeCommandResult = when (command.type) {
        ...
        WearBridgeCommandType.SyncSettings -> WearBridgeCommandResult(command.id, success = true, message = "Settings sync reserved")

...
class AndroidWearMedia3PlaybackController(context: Context) : MediaPlaybackController {
    ...
    private fun updateState(state: PlaybackState) { _session.update { it.copy(state = state, positionMillis = player.currentPosition, durationMillis = player.duration.takeIf { d -> d > 0 }) } }
    private fun updatePosition() { _session.update { it.copy(positionMillis = player.currentPosition, durationMillis = player.duration.takeIf { d -> d > 0 }) } }

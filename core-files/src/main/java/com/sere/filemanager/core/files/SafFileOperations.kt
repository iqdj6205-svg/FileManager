...
class SafFileOperations(
    ...
    private fun DocumentFile.toFileItem(): FileItem = FileItem(name = name ?: "Unnamed", path = uri.toString(), type = if (isDirectory) FileItemType.Directory else com.sere.filemanager.core.model.FileTypeClassifier.fromMime(type), sizeBytes = if (isFile) length() else null, modifiedAt = lastModified().takeIf { it > 0L }?.let(Instant::ofEpochMilli))
}

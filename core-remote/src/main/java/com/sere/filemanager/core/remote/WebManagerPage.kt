...
object WebManagerPage {
    fun render(status: RemoteServerSession, currentPath: String = "/sdcard", message: String? = null): String {
        ...
        return """
            ...
            <style>:root{color-scheme:dark;font-family:system-ui,-apple-system,BlinkMacSystemFont,sans-serif}body{margin:0;background:#09090b;color:#fafafa}header{padding:18px;background:linear-gradient(135deg,#2563eb,#7c3aed)}main{padding:16px;max-width:960px;margin:0 auto}.card{border:1px solid #27272a;border-radius:18px;padding:16px;margin:12px 0;background:#18181b}.row{display:flex;gap:8px;flex-wrap:wrap;align-items:center}input,button{border-radius:12px;border:1px solid #3f3f46;padding:10px 12px;background:#09090b;color:#fafafa}button{background:#2563eb;border:none;cursor:pointer}button.danger{background:#dc2626}button.secondary{background:#3f3f46}code{background:#27272a;padding:2px 6px;border-radius:6px}a{color:#93c5fd}.entry{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;padding:10px;border-bottom:1px solid #27272a}.muted{color:#a1a1aa;font-size:12px}</style></head>
            <body><header><h1>FileManager Remote</h1><p>Watch storage browser · local network only</p></header><main>
            <section class="card"><h2>Connection</h2><p>State: <b>${status.state}</b></p><p>URL: <code>${status.url.orEmpty()}</code></p><p>PIN: <code>$pin</code></p>${message?.let { "<p>$it</p>" }.orEmpty()}</section>
            ...
            async function renameItem(path,oldName){const name=prompt('New name',oldName);if(!name)return;await api('/api/rename?path='+encodeURIComponent(path)+'&name='+encodeURIComponent(name),{method:'POST'});loadList()}
            async function deleteItem(path,name){if(!confirm('Delete '+name+'?'))return;await api('/api/delete?path='+encodeURIComponent(path),{method:'POST'});loadList()}

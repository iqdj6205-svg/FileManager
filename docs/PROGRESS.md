...
- Wear rename action now opens an editable input screen instead of silently renaming to `copy-*`; follow-up compile-risk review fixed `item` scope shadowing.
...
- Phone media library, preview, image preview, and storage roots now use the shared Phone UI scaffold/cards for consistent spacing/layout.

...
- `[~]` Remote server status/list/download consistency — `1ff1d86` publishes embedded start/stop session to shared status store; `b67ef45` exposes reachable local IP URL and blocks offline start; `2f3d5fb` enforces local-network start when local-only is enabled; `pending` shows active config in web manager; endpoint verification still pending.

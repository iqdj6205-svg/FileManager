...
- `[~]` Remote server status/list/download consistency — `1ff1d86` publishes embedded start/stop session to shared status store; `b67ef45` exposes reachable local IP URL and blocks offline start; `pending` enforces local-network start when local-only is enabled; endpoint verification still pending.
